...
# ==========================================
# 🖨️👑 ENGINE 10: PRO PRINT STUDIO (Top-Center Normal, Top-Left Small, Horizontal Rows Photos)
# ==========================================
...
def process_print_studio():
    try:
        ...
        if job_type == 'id_card':
            id_w, id_h = 1150, 730  # Increased size for better visibility
            ...
            if paper_size in ['4x6', '5x7']:
                # Small photo papers: Always Top-Left baseline, One below another
                ...
                # 🎯 A4 SHEET CONFIGURATION ENGINE
                if lamination_mode:
                    # 1. Lamination Mode -> Side-by-Side & Perfectly Centered Horizontal
                    gap = 60
                    mid_x = canvas_w // 2
                    fx = mid_x - id_w - (gap // 2)
                    bx = mid_x + (gap // 2)
                    ...
                    # 2. 🎯 NORMAL MODE -> TOP-CENTER, ONE BELOW ANOTHER
                    gap = 80
                    fx = (canvas_w - id_w) // 2  # Horizontal Center Axis Lock
                    fy = margin_y
                    bx = fx  # Aligned directly underneath on same X axis
                    by = fy + id_h + gap
            ...
            # 🎯 PHOTO STUDIO ENGINE: "Aadvya Olit Grid" Flow (Left to Right Row Compilation)
            ...
            # This ensures photos align perfectly in horizontal rows
        ...
        img_byte_arr = io.BytesIO()
        canvas.save(img_byte_arr, format='JPEG', quality=100, dpi=(300, 300))
        img_byte_arr.seek(0)
